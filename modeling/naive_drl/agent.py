...
class SimpleDRLAgent:
    ...
    def get_immed_reward(self, buysell, cost_bid, cost_ask, trade_bid, trade_ask):
        ...
        if trade_mid_price > cost_mid_price:
            # Price is up
            if buysell == 'B':
                ...
                reward = -1
        elif trade_mid_price < cost_mid_price:
            # Price is down
            if buysell == 'B':
    ...
    def train(self):
        ...
        for epoch in range(self.config['epochs']):
            for epi_idx in range(self.config['total_no_epi']):
                ...
                print('Epoch:{} Episode:{}. The training loss is {}. The total reward is {}'.format(epoch+1, epi_idx+1, losses, accum_rewards[t]))
            
            ### enf of one epoch ##
            epochs_total_rewards.append(accum_rewards[-1])
        
        return accum_rewards, np.array(epochs_total_rewards)